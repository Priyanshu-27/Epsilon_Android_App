...
import { getToken, removeToken, StoreToken, Store_S_ID, get_S_ID, remove_S_ID } from '../services/Asyncstorage'
import { useEffect, useState } from 'react'
import GardianRegistration from './GardianRegistration'
...
const ProfileforStudent = () => {
  ...
  useEffect(() => {
    ...
    const getProfile = async () => {
      const result = await axios.get(`http://10.0.2.2:8000/${JSON.parse(S_id)}/Profile`);
      setData(result.data);
      // setisData(true);
    };
    isFocused && getProfile(S_id)
  }, [isFocused]);
    ...
    <ScrollView>
      <SafeAreaView style={styles.container}>
        <View style={{ marginTop: 20, elevation: 6, height: 50, marginBottom: 30, alignItems: 'center', justifyContent: 'center' }} >
          <TouchableRipple onPress={() =>{ if(S_id){
                                          navigation.navigate("UpdateStudentProfile")
                                      }   
                                    else{navigation.navigate('StudentEditProfile')}}}>
        ...
        <View style={styles.userInfoSection}>
          <View style={{ flexDirection: 'row', marginTop: 15 }}>
            <Avatar.Image source={{ uri: `http://10.0.2.2:8000${data.profile_picture}` }} size={80} />
        ...
        <View style={styles.userInfoSection}>
          ...
          <View>
            <Text style={{ fontWeight: '600', color: 'black', marginBottom: 20, marginTop: 15 }}>Guardian information</Text>
            <View>
              ...
              <View style={styles.row}>
                ...
                <Text style={{ color: '#777777', marginLeft: 20 }}>{data.guardian_email}</Text>
              </View>
        ...
        <View style={{ marginTop: 40 }}>
          ...
          <TouchableRipple onPress={() => { navigation.navigate('GardianRegistration') }}>
            <View style={styles.menuItem}>
              <Ionicons name="create-outline" color='#FF6347' size={25} />
              <Text style={styles.menuITemText}>Registor Gardian  </Text>
            </View>
          </TouchableRipple>

          <TouchableRipple onPress={() => {navigation.navigate("HelpForStudent")}}>
            <View style={styles.menuItem}>
          ...
          <TouchableRipple onPress={NotificationFunc}>
            <View style={styles.menuItem}>
              <Ionicons name="help-circle-outline" color='#FF6347' size={25} />
              <Text style={styles.menuITemText}>Change Student </Text>
            </View>
          </TouchableRipple>
